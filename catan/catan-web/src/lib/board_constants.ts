export const HEX_SIDE_LENGTH = 100;
export const HEX_WIDTH = Math.sqrt(3) * HEX_SIDE_LENGTH;
export const HEX_HEIGHT = HEX_SIDE_LENGTH * 2;
export const HEX_CENTER_X = HEX_WIDTH / 2;
export const HEX_CENTER_Y = HEX_HEIGHT / 2;
export const HEX_ROW_HEIGHT = HEX_SIDE_LENGTH + HEX_SIDE_LENGTH / 2;
export const BOARD_MARGIN_TOP = HEX_SIDE_LENGTH / 4 + HEX_HEIGHT / 2;
export const BOARD_MARGIN_SIDE = HEX_SIDE_LENGTH / 4 + HEX_WIDTH;

export const ROLL_NUMBER_RADIUS = HEX_SIDE_LENGTH / 3;
export const FONT_SIZE = HEX_SIDE_LENGTH / 4;
export const PROBABILITY_RADIUS = ROLL_NUMBER_RADIUS / 12;
export const PROBABILITY_MARGIN = PROBABILITY_RADIUS * 3;

export const CORNER_RADIUS = ROLL_NUMBER_RADIUS / 3;
export const CORNER_DELTA_HEIGHT = HEX_SIDE_LENGTH / 4;
export const CORNER_START_X = BOARD_MARGIN_SIDE + HEX_CENTER_X;
export const CORNER_START_Y = BOARD_MARGIN_TOP + HEX_HEIGHT / 4;

export const EDGE_WIDTH = CORNER_RADIUS;
export const EDGE_HEIGHT = 0.65 * HEX_SIDE_LENGTH;
export const EDGE_START_X = BOARD_MARGIN_SIDE + HEX_CENTER_X + HEX_WIDTH / 4;
export const EDGE_START_Y = BOARD_MARGIN_TOP + HEX_SIDE_LENGTH / 4;

export const TRADE_RADIUS = (ROLL_NUMBER_RADIUS * 2) / 4;
export const TRADE_LINE_WIDTH = EDGE_WIDTH / 2;
export const TRADE_LINE_HEIGHT = EDGE_HEIGHT;
