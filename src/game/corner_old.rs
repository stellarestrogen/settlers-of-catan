...
impl CornerHolder {
    ...
    pub fn calc_distance(&self, first: u32, second: u32) -> u32 {
        // current corner that we iterate with will always be either on top of or to the left of the destination corner. it can either be below or above it.
        ...
        while current_row != dest_row && current_row_idx != dest_row_idx {
            ...
            let right = current_row_idx > dest_row_idx; // true if we are to the right of the destination.
            
            // this will cause the current row to move in a snake-like pattern, giving the shortest distance between the 2 corners. 
            if current_row > dest_row && self.can_traverse_up(current_idx) { // are we below the destination, and can we traverse up?
                current_row -= 1;
                distance += 1;
            } else if current_row < dest_row && self.can_traverse_down(current_idx) { // are we above the destination, and can we traverse down?
                current_row +=1;
                distance += 1;
            } else { // otherwise, move horizontally towards the destination.
