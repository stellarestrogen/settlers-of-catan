...
impl Board {
    ...
    pub fn create_tile_board(&mut self, longest: u32, shortest: u32) {
        // abusing math :3
        ...
        for y in 0..self.row_count {
            ...
            if y > self.row_count/2 {
                row_resource_count = longest + longest - shortest - y; // some more math fuckery
            }
            // tiles first
    ...
    pub fn create_board(&mut self, size: BoardSize) {
        ...
        match size {
            BoardSize::Hexagonal { shortest, longest } => {
                // inputs should be sanitized to not give invalid results, but we check just in case
                if shortest >= longest { panic!("Invalid board size!\n") }
